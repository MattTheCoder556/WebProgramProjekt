...
$connect = new mysqli("localhost","root","","dogwalkxampp");
    ...
    switch (key($operation)) {
        ...
        case 'update':
            if (isset($_POST['submit'])) {
                $sql = "UPDATE dogs SET ";
                $sql .= implode(', ', $sql_array);
                $sql .= " WHERE dogs_id = '". $operation['update'] ."'";
            }
            else {
                $sql = "SELECT dogs_id, dog_pic, dog_name, dog_breed, dog_age, dog_bday, dog_gender FROM dogs WHERE dogs_id = " . $operation['update'];
            ...
            break;
        case 'delete':
            $sql = "DELETE FROM dogs WHERE dogs_id = '". $operation['delete'] ."'";
            break;
    }
    // Send sql to server
