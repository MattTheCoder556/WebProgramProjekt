    ...
    switch (key($operation)) {
        ...
        case 'update':
            if (isset($_POST['submit'])) {
                ...
                $sql = "SELECT u_id, u_pic, u_fname, u_lname, u_email, u_pass, u_phone, u_address, walk_switch, is_banned FROM users WHERE u_id = ?";
                $stmt = $pdo->prepare($sql);
...
$sql = "SELECT u_id, u_pic, u_fname, u_lname, u_email, u_pass, u_phone, u_address, walk_switch, is_banned FROM users";
$stmt = $pdo->query($sql);
        ...
        <form method="POST">
            <table class="admin__table">
                ...
                foreach ($u as $key => $value) {
                    ...
                    echo '<tr>
                                    <td>' . $value['u_id'] . '</td>
                                    <td><img width="80px" height="80px" src ="ProfPic/' . $value['u_pic'] . '"></td>
                                    <td>' . $value['u_fname'] . '</td>
                                    <td>' . $value['u_lname'] . '</td>
                                    <td>' . $value['u_email'] . '</td>
                                    <td>' . $value['u_pass'] . '</td>
    ...
    <section class="block">
        <div class="page-wrapper">
            ...
            <form class="admin__update" method="POST">
                ...
                <div class="update__item"><input placeholder="Photo" type="file" name='u_pic' value="<?php if(isset($update)){echo $update['u_pic'];} ?>"></div>
                ...
                <div class="update__item"><label for = "bann">Banned?</label><input placeholder="Banned" type="checkbox" id = "bann" name='is_banned' value="<?php if(isset($update)){echo $update['is_banned'];} ?>"></div>
                <button class="admin__table-new" type="submit" name="submit" value="true">Send</button>
            </form>
        </div>
    </section>
<?php } ?>
<p><a href = "adminDogs.php">To admin Dogs</a></p>
